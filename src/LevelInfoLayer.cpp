...
class $modify(MessageChecker_Info, LevelInfoLayer) {
    struct Fields {
    ...
    void checkMessages() {
        ...
        if (Mod::get()->getSettingValue<bool>("stop-notifications")) {
            log::info("notifications globally disabled (stop-notifications enabled)");
            return;
        }
        ...
        req->setResponseCallback(this, SEL_HttpResponse(&MessageChecker_Info::onMessageResponse));
    ...
    void onScheduledTick(float) {
        ...
        try {
            ...
            // Unclamped according to your request
    ...
    bool init(GJGameLevel* level, bool challenge) {
        ...
        log::info("LevelInfoLayer init");

        // No bootChecked check, no onScheduledTick(0) call per your instructions
        log::info("re-running the schedule");
        this->schedule(schedule_selector(MessageChecker_Info::onScheduledTick), 1.0f);
